<template>
	<div>
		<p>Start date: <em>{{ inputValue }}</em></p>
	...
	setup(props, { emit }) {
		...
		watch(inputValue, (newValue) => {
			...
			if (selectedDate < constantDate) {
				errorMessage.value = `Start date cannot be before ${constantDate.toDateString()}`;
			} else if (selectedDate > today) {
				errorMessage.value = 'Start date cannot be in the future';
			} else {
				errorMessage.value = '';
				emit('start-date', inputValue.value);
			}
		});
